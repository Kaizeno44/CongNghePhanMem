...
def check_order_api(request):
    query = request.GET.get('query', '').strip()
    if not query:
        return JsonResponse({"error": "Vui lòng nhập số điện thoại hoặc email"}, status=400)

    orders = Order.objects.filter(phone_number=query) | Order.objects.filter(customer__email=query)
    if not orders.exists():
        return JsonResponse({"error": "Không tìm thấy đơn hàng"}, status=404)

    order_list = [{
        "id": order.id,
        "customer": order.customer.username if order.customer else "Khách vãng lai",
        ...
        "voucher": order.voucher.code if order.voucher else "Không có",
        "notes": order.notes if order.notes else "Không có ghi chú"
    } for order in orders]
